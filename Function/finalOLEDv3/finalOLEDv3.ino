...
/**
 * 绘制光线趋势图界面 (修改版 - 简化护眼提示)
 * 功能：显示光线传感器数据趋势，用于暂停阅读时的界面
 * 特色：包含电池图标、趋势曲线、装饰元素，以及简洁的护眼提示
 */
void drawTrendGraph() {
...
/**
 * 蜂鸣器提示音函数 (适配新版ESP32 Arduino Core)
 * 参数：duration - 鸣叫持续时间 (毫秒)
 * 功能：播放指定时长的提示音
 * 使用新版LEDC API控制蜂鸣器
 */
void beep(int duration = 100) {
  ledcWrite(piezoPin, 127);     // 设置PWM占空比为50% (127/255)
  delay(duration);              // 持续指定时间
  ledcWrite(piezoPin, 0);       // 关闭蜂鸣器
 ...
 * 功能：根据用户阅读习惯智能调整番茄钟时长
 * 算法特点：
 *   - 平滑调整：使用学习率控制变化幅度，避免剧烈跳跃
 *   - 双向记录：所有会话都计入历史数据，不偏向失败案例
 *   - 渐进式：根据完成率进行差异化调整
 *   - 趋势感知：考虑近期表现趋势
 */
void saveAdaptivePomodoro(unsigned long sessionMillis) {
  ...
  // 算法参数配置
  const float LEARNING_RATE = 0.15;      // 学习率：控制调整幅度 (0.1-0.3)
  const float SUCCESS_GROWTH = 1.05;     // 成功完成时的增长因子 (5%增长)
  const float PARTIAL_GROWTH = 1.02;     // 部分完成时的小幅增长 (2%增长)
  const float ADJUST_RATE = 0.8;         // 未完成时的调整系数
  ...
  if (completionRate >= 1.0) {
    ...
    if (completionRate >= 1.2) {
      ...
      Serial.println("[Pomodoro] Excellent! Target increased (超额完成)");
    } else {
      // 正常完成：小幅增加目标
      adaptivePomodoroMillis = (unsigned long)(adaptivePomodoroMillis * PARTIAL_GROWTH);
      Serial.println("[Pomodoro] Good! Target slightly increased (正常完成)");
    }
  } else if (completionRate >= 0.8) {
    // 情况B：80%-100%完成率，表现良好，维持当前目标
    // 微调向历史平均值靠拢
    adaptivePomodoroMillis = (unsigned long)(LEARNING_RATE * historicalAverage + 
                                           (1 - LEARNING_RATE) * adaptivePomodoroMillis);
    Serial.println("[Pomodoro] Stable performance, minor adjustment (表现稳定)");
  } else if (completionRate >= 0.6) {
    // 情况C：60%-80%完成率，需要适度调整
    ...
    adaptivePomodoroMillis = (unsigned long)(LEARNING_RATE * targetAdjustment + 
                                           (1 - LEARNING_RATE) * adaptivePomodoroMillis);
    Serial.println("[Pomodoro] Moderate adjustment needed (适度调整)");
  } else {
    // 情况D：完成率低于60%，需要明显降低目标
    unsigned long targetAdjustment = (unsigned long)(historicalAverage * 0.7);
    adaptivePomodoroMillis = (unsigned long)(LEARNING_RATE * targetAdjustment + 
                                           (1 - LEARNING_RATE) * adaptivePomodoroMillis);
    Serial.println("[Pomodoro] Significant adjustment needed (显著调整)");
  }
  ...
  Serial.println("本次会话: " + String(sessionMillis / 60000) + "分" + String((sessionMillis / 1000) % 60) + "秒");
  Serial.println("完成率: " + String(completionRate * 100, 1) + "%");
  Serial.println("历史平均: " + String(historicalAverage / 60000) + "分钟");
  Serial.println("新目标: " + String(adaptivePomodoroMillis / 60000) + "分钟");
  Serial.println("总会话数: " + String(adaptiveSessionCount));
  Serial.println("==================");
}
